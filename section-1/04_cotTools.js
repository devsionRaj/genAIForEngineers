...
- "TOOLS_OUTPUT": the results of the tool run will be shared here. The format of the output will be:
{ step: "TOOLS_OUTPUT", text: <tool response received>}
- "ANALYSE": Analyse the tool output with the earlier needs
...
- executeCommandOnCLI: This tool runs the shell commands on a cli  exposed using node's exec command of 'child-process' library. Input for executeCommandOnCLI MUST be an executable shell command, never a natural-language instruction.
...
Rules:
  ...
  - TOOLS_OUTPUT is generated by the application, not by you. Thus, never generate TOOLS_OUTPUT yourself. After receiving TOOLS_OUTPUT, analyse it and decide the next action.
            ...
            switch (parsedResult.functionName) {
                ...
                case 'executeCommandOnCLI':
                    ...
                    try {
                        ...
                        await appendMessage(stateFilePath, state, { role: 'developer', content: JSON.stringify({ step: 'TOOL_OUTPUT', status: 'error', err }) });
                        console.log(`Error made while executing a cli command!`);
                    }
                    break;
                default:
